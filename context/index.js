...
class CheckoutPage extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const hasDeliveryCountryChanged =
      prevState.deliveryCountry !== this.state.deliveryCountry;
    const hasDeliveryRegionChanged =
      prevState.deliveryRegion !== this.state.deliveryRegion;
    ...
    if (
      hasDeliveryCountryChanged ||
      (hasDeliveryRegionChanged && this.props.checkout)
    ) {
    ...
    if (
      prevState["fulfillment[shipping_method]"] !==
   ...
   */
  generateToken() {
    const { cart, dispatchGenerateCheckout, dispatchGetShippingOptions } =
      this.props;
    ...
    return dispatchGenerateCheckout(cart.id)
      ...
      .catch((error) => {
        console.log(
          "error caught in checkout/index.js in generateToken",
          error
        );
      });
  }
  ...
  captureOrder(e) {
    ...
    const line_items = this.props.checkout.live.line_items.reduce(
      (obj, lineItem) => {
                  ...
                  <div className='mb-5'>
                    <ShippingForm
                      ...
                      shippingPostalZipCode={
                        this.state["shipping[postal_zip_code]"]
                      }
            ...
            <div className='col-12 col-lg-5 col-md-10 offset-md-1'>
              <div className='bg-brand200 p-5 checkout-summary'>
                ...
                <div className='pt-3 borderbottom border-color-gray400'>
                  {(checkout.live ? checkout.live.line_items : []).map(
                    (item, index, items) => {
                ...
                <div className='py-3 borderbottom border-color-black'>
                  {[
                    ...
                    },
                    {
                      name: "Tax",
                      amount: checkout.live
                        ? checkout.live.tax.amount.formatted_with_symbol
                        : "",
                    },
                    {
                      name: "Shipping",
                    ...
                    },
                    {
                      ...
                      amount:
                        checkout.live &&
                        checkout.live.discount &&
                        checkout.live.discount.code
                ...
                <div className='d-flex justify-content-between align-items-center mb-2 pt-3'>
                  ...
                  <p className='text-right font-weight-semibold font-size-title'>
                    ${" "}
                    {checkout.live
                      ? checkout.live.total.formatted_with_code
                      : ""}
                  </p>
                </div>
...
export default withRouter(
  connect(
    ({ checkout: { checkoutTokenObject, shippingOptions }, cart }) => ({
